document.addEventListener("DOMContentLoaded", async () => {
  ...
  const abrirModal = () => { overlay.classList.remove("hidden"); modal.classList.remove("hidden"); modal.classList.add("flex"); };
  const cerrarModal = () => { overlay.classList.add("hidden"); modal.classList.add("hidden"); modal.classList.remove("flex"); form.reset(); listaCilindrosInspeccion.innerHTML = ""; };
  ...
  const abrirInspeccionParaPedido = async (pedido) => {
    ...
    detalles.forEach((d, idx) => {
      ...
      opt0.value = ""; opt0.textContent = "Seleccione un cilindro de reemplazo";
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    for (const d of detalles) {
      ...
      radios.forEach(r => { if (r.checked && r.value === 'replace') enCondiciones = false; });
      ...
      if (enCondiciones) {
        ...
        cilindros.push({ cilindro_id: d.id_cilindro, en_condiciones: false, cilindro_reemplazo_id: repId, comentarios: obs || undefined });
      }
    }
