...
exports.getPermisos = async (req, res) => {
  const { id } = req.params;
  console.log("rolId recibido del frontend:", id);
  try {
...
exports.updatePermisos = async (req, res) => {
  ...
  const modulosAfectados = permisos.map(p => p.split('.')[0]);
  const modulosUnicos = [...new Set(modulosAfectados)];
  ...
  try {
    ...
    await registrarHistorial(
      ...
      `Actualizó los permisos del rol "${nombreRol}". Módulos afectados: ${modulosUnicos.join(', ')}.`,
    );
...
exports.getPermisos = async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT permisos FROM rol WHERE id = $1', [id]);
    console.log("Permisos BD:", result.rows[0].permisos); 
    if (result.rows.length === 0) {
      return res.status(404).json({ message: 'Rol no encontrado' });
    }
    res.json({ permisos: result.rows[0].permisos });
